...
JAVA数据类型  
    基本数据类型（4类8种）  
        ...
        &ensp;byte:使用一个字节存储，因此范围是-128 ~ 127  
        &ensp;short:使用两个字节存储，因此范围是-32768 ~ 32767  
        &ensp;int：使用4个字节存储，因此范围是正负21亿  
        &ensp;long：使用8个字节存储，因此范围是.....一般也超不了这个范围  
        &ensp;&ensp;注意：在使用整型类型的时候，默认都是int，**如果需要使用long类型，必须要在数字后面加L**,建议使用大写，小写会和1混淆
        - 浮点类型:float double  
        &ensp;float:单精度  
        &ensp;double：双精度  
        - 字符类型:char  
        - 布尔类型:boolean  
        &ensp;只有true和false两值，在存储的时候占一位  
##### 整型数字类型
- 整型常量
    -十进制
    -八进制（几乎用不上），要求以0开头
